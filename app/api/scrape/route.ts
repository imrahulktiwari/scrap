...
function extractPlayStoreId(input: string): string | null {
  ...
  try {
    ...
    if (url.hostname === 'play.google.com' || url.hostname.endsWith('.play.google.com')) {
...
export async function POST(request: Request) {
  try {
    ...
    if (iosId !== null) {
      ...
      const scraper = (appStore as any).app ? appStore : ((appStore as any).default || appStore);

      if (typeof scraper.app !== 'function') {
        console.error("app-store-scraper structure: ", typeof appStore, appStore);
        return NextResponse.json(
      ...
      return NextResponse.json({
        ...
        developerEmail: "", // App Store API does not provide dev support email
        ...
        priceText: app.price === 0 || app.free ? "Free" : `${app.price} ${app.currency || "USD"}`,
        free: app.free,
        released: app.released ? new Date(app.released).toLocaleDateString() : "",
    ...
    if (!playId) {
      return NextResponse.json(
        { error: "Could not extract a valid Google Play package ID (e.g. com.instagram.android) or App Store ID (e.g. 310633997) from your input." },
        { status: 400 }
      );
    }

    const scraper = (gplay as any).app ? gplay : ((gplay as any).default || gplay);
    ...
    if (message.includes("not found") || message.includes("404")) {
      return NextResponse.json(
        { error: "App not found. Please check that the URL or application ID is correct and publicly available." },
        { status: 404 }
      );
    }
    return NextResponse.json(
      { error: error.message || "An unexpected error occurred while scraping the app details." },
      { status: 500 }
    );
  }
}
